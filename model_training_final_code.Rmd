...
Using caret: No tuning grid because no tuning parameter. 
```{r}
...
set.seed(1)
model.lda <- train(x = train.data[,c(1:11)],
                   y = train.data$stroke,
                   method = "lda",
                   metric = "ROC", #AUC
                   trControl = ctrl)
...
model.lda <- train(x = stroke_df[trRow,2:11],
                   y = stroke_df$stroke[trRow],
                   method = "lda",
                   metric = "ROC",
                   trControl = ctrl)
